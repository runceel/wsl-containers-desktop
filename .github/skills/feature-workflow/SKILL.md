...
| 4 | 実装(TDD) | Red→Green→Refactorを1振る舞いずつ繰り返す | `tdd-red.agent.md`（[ADR-0008](../../../docs/adr/0008-expand-model-routing-to-mechanical-workflow-steps.md)の条件でFlash既定）→ `tdd-green.agent.md`（[ADR-0016](../../../docs/adr/0016-set-sonnet-5-baseline-and-route-green-to-flash.md)によりFlash既定・新規設計判断時はベースラインへエスカレーション）→ `tdd-refactor.agent.md`（ベースライン） |
...
3. **テスト作成**: `rubber-duck` のレビューで「仕様の抜け漏れ」の指摘が解消済み。加えて、
   具体的な入出力・アサーション値・エッジケース（表形式等）まで確定していること
   （[ADR-0008](../../../docs/adr/0008-expand-model-routing-to-mechanical-workflow-steps.md) /
   [ADR-0016](../../../docs/adr/0016-set-sonnet-5-baseline-and-route-green-to-flash.md)により、
   ここが確定していないと後続のTDD Red/Greenフェーズを低コストモデルで進められないため）。
