...
- **稼働中コンテナのリソース使用量リスト**: `ContainerStats`（`DashboardContainerStatsRowViewModel` の
  ...
  「詳細」「ログ」を開くボタンを持つ。
- **空状態**: `IsStatsEmpty`（`ContainerStats.Count == 0 && StatsErrorMessage is null`）が真のとき、
  稼働中コンテナがない旨の空状態テキストを表示する。
...
- **`ContainersViewModel` への直接結合**: 行アクションが `ContainersViewModel` の具体型と公開コマンドに
  ...
  疎結合化する余地がある。
- **空状態は取得完了後にのみ厳密**: `IsStatsEmpty` は「件数0かつエラーなし」で真になるため、初回ロード中や
  統計クリア直後〜取得完了前の一瞬、「稼働中コンテナなし」を表示し得る。必要になれば
  ロード状態フラグ（例: `HasStatsLoaded`）で区別することを検討する。
