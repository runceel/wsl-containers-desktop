...
- **稼働中コンテナのリソース使用量リスト**: `ContainerStats`（`DashboardContainerStatsRowViewModel` の
  ...
  「詳細」「ログ」を開くボタンを持つ。
  - ヘッダーは`ListView.HeaderTemplate`内ではなく、一覧と同じ親Gridに置く兄弟要素である。
    読み込み中は一覧と同時に非表示にする。
  - Application resourcesの`DashboardStatsTableColumnLayout`をヘッダーと行で共有し、名前・CPU・
    メモリの列幅を同期する。`TableColumnSplitter`は右側全列の余力を近い順に使って境界を移動し、
    星幅の名前列をリサイズ後も星幅として維持する。変更した幅はページ遷移後も同じプロセス内で維持する。
  - メタデータ領域と幅`192`の詳細/ログ操作領域を分離し、ヘッダーと行で同じ固定幅を予約する。
...
- **表形式 ListView の行幅**: 稼働中コンテナ行は `TableListViewItemStyle` と
  `HorizontalAlignment="Stretch"` の行Gridで横幅いっぱいに伸ばす。兄弟ヘッダーと行は
  `DashboardStatsTableColumnLayout`と同じ固定アクション幅を共有し、`ListViewItem` 既定paddingではなく
  ヘッダー/行Gridのpaddingで列位置を揃える。
...
- **`ContainersViewModel` への直接結合**: 行アクションが `ContainersViewModel` の具体型と公開コマンドに
  ...
  疎結合化する余地がある。
- **検査の並列数**: `DashboardService` はコンテナ詳細を最大4件並列に取得する。
  `WslcCliVolumeRuntimeClient`と`WslcCliNetworkRuntimeClient`も、共通の
  `BoundedConcurrencyInspector`を使ってボリューム・ネットワーク詳細を最大4件ずつ並列に取得する。
  いずれも取得結果は入力一覧と同じ順序を維持する。
