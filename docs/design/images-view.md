...
- 表形式の`ListView`は`TableListViewItemStyle`で行コンテンツを横方向いっぱいに広げ、ヘッダーGridと
  行Gridの水平paddingを揃える。メタデータ領域と幅`224`の起動/削除操作領域を分離し、ヘッダーと行で
  同じ固定幅を予約する。
- Application resourcesの`ImagesTableColumnLayout`をヘッダーと行で共有し、表示名・イメージID・
  サイズ・作成日時の列幅を同期する。`TableColumnSplitter`は右側全列の余力を近い順に使って境界を移動し、
  4つの星幅列をリサイズ後も星幅として維持する。変更した幅はページ遷移後も同じプロセス内で維持する。
