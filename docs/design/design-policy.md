...
- Containers、Images、Volumes、Networks、Dashboard statsは、標準`ListView`とヘッダー/行の`Grid`で
  表を構成する。
- 各一覧はプロセス中に1つの`TableColumnLayout`をApplication resourcesから共有し、
  `TableLayoutBehavior`がヘッダーと実体化済みの行へ同じ列幅を適用する。変更した幅はページ遷移後も
  同じプロセス内で維持し、アプリ再起動時に既定値へ戻す。
- `TableColumnSplitter`はポインタードラッグと左右キーの両方を受け付ける。境界を右へ動かすと、
  左側全列の拡張余地と右側全列の縮小余地をそれぞれ境界に近い列から順に使い、左へ動かすと
  左側全列の縮小余地と右側全列の拡張余地を同じ順序で使う。最小幅・最大幅を超える入力は
  最も近い可動限界へクランプし、星幅の列は変更後も星幅を維持する。
- 各表はリサイズ可能なメタデータ領域と数値固定幅のアクション領域を分離する。メタデータ領域は
  表示幅でクリップし、アクション領域へ重ねず、一覧全体の横スクロールは使わない。
- スプリッターはTabフォーカス、標準フォーカス表示、`AutomationId`、隣接列を示すローカライズ済みの
  アクセシブル名を持つ。

詳細は [ADR-0018](../adr/0018-adopt-resizable-table-layouts-with-fixed-action-rails.md) を参照。
...
コンテナ行右端の固定操作領域には状態に応じたPlay/Stopアイコンと`…`メニューを置き、
主要な起動・停止を1操作で実行できるようにする。
